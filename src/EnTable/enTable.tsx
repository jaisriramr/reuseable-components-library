...
const EnTable: FC<EnTableProps> = ({ rows }) => {
  ...
  const pageLimit = 10
  const TotalPageNum: number = Math.ceil(Number(rows?.length) / pageLimit);
    ...
    <Fragment>
      ...
      >
        ...
        />
          ...
          <Pagination count={Math.ceil(Number(rows?.length) / pageLimit)} siblingCount={0} defaultPage={1} boundaryCount={1} page={page} shape="rounded"  onChange={handlePaginationChange} />
        ...
        <KeyboardDoubleArrowRightIcon
          ...
          sx={{
            display: `${page === Math.ceil(Number(rows?.length) / pageLimit) ? "none" : "block"}`,
            cursor: "pointer",
            height: "18px",
            width: "18px",
          }}
